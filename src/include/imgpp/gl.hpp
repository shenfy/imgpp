...
inline ExternalFormat TranslateFromES20(ExternalFormat external) {
  if (external == EXTERNAL_SRGB_EXT) {
    return EXTERNAL_RGB;
  } else if (external == EXTERNAL_SRGB_ALPHA_EXT) {
    return EXTERNAL_RGBA;
  }
  return external;
}

inline ExternalFormat TranslateToES20(ExternalFormat external) {
  if (external == EXTERNAL_RGB) {
    return EXTERNAL_SRGB_EXT;
  } else if (external == EXTERNAL_RGBA) {
    return EXTERNAL_SRGB_ALPHA_EXT;
  }
  return external;
}

inline TypeFormat TranslateFromES20(TypeFormat type) {
...
inline TypeFormat TranslateToES20(TypeFormat type) {
  if (type == TYPE_F16) {
    return TYPE_F16_OES;
  }
  return type;
